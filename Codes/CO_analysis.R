...
tot_num_wells <- reg_data_CO[,wdid] %>% unique() %>% length()

hp_changes_num <- reg_data_CO[,.(hp_dev=pump_hp-mean(pump_hp)),by=wdid] %>%
  .[hp_dev!=0,wdid] %>% unique %>% length()

#/*=================================================*/
#' # Summary statistics
#/*=================================================*/
mean_ir_CO <- reg_data_CO[,mean(water_use)] %>% round(digits=2)
mean_gpm_CO <- reg_data_CO[,mean(gpm)] %>% round(digits=2) %>%
  round(digits=2)
min_gpm_CO <- reg_data_CO[,min(gpm)] %>% round(digits=2) %>%
  round(digits=2)
max_gpm_CO <- reg_data_CO[,max(gpm)] %>% round(digits=2) %>%
  round(digits=2)
mean_etw_CO <- reg_data_CO[,mean(etw)] %>% round(digits=2) %>% round(digits=2)
mean_precip_CO <- reg_data_CO[,mean(precip)] %>% round(digits=2) %>% round(digits=2)

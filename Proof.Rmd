...
This work is supported by the National Institute of Food and Agriculture, U.S. Department of Agriculture, under award number 2016-68007-25066, "Sustaining agriculture through adaptive management to preserve the OA under a changing climate."
...
```{r usgs-close, fig.cap = "Irrigation wells that are near and far from USGS monitoring wells", results = "hide"}
# /*=================================================*/
#' # Spatial overlap of USGS wells and irrigation wells
# /*=================================================*/

ir_usgs <- readRDS(here("Proof/Data/ir_usgs.rds"))

wells_loc_sf <- readRDS(here("Proof/Data/wells_loc_sf.rds")) %>%
  left_join(ir_usgs, by = "wdid") %>%
  mutate(type = ifelse(is.na(site_no), "Far", "Near")) %>%
  select(type)

CO_gwl_sf <- readRDS(here("Proof/Data/CO_gwl_sf.rds")) %>%
  distinct(geometry) %>%
...
ggsave(g_near, file = here("Proof/Figures/wells_near_USGS.pdf"), height = 5.583, width = 5.583)
...
```{r fe-reg-CO-bias, tab.cap = "The magnitude of potential bias when the endogeneity due to the declining block structure is untreated"}
